...
class ProcessPaymentView(APIView):
    def post(self, request, format=None):
        ...
        if newTransaction.is_success or newTransaction.transaction:
            for cart_item in cart_items:
                update_producto = Product.objects.get(id=cart_item.product.id)

                # Encontrar cantidad despues de compra
                quantity = int(update_producto.quantity) - int(cart_item.count)

                # Obtener cantidad de producto por vender
                sold = int(update_producto.sold) + int(cart_item.count)
            ...
            for cart_item in cart_items:
                try:
                    # Obtener el producto de la instancia
